...
func New(containerCollectAll bool) schedulers.Scheduler {
	sch := &Scheduler{
...
func (s *Scheduler) Schedule(configs []integration.Config) {
	for _, config := range configs {
		if util.CcaUseBareConfigs() && s.containerCollectAll && config.IsBareConfig() {
			// (NOTE: when removing util.CcaUseBareConfigs, integrate this with toSources)
...
func (s *Scheduler) Unschedule(configs []integration.Config) {
	for _, config := range configs {
		if util.CcaUseBareConfigs() && s.containerCollectAll && config.IsBareConfig() {
			// (NOTE: when removing util.CcaUseBareConfigs, integrate this with the case below)
